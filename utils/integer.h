...
namespace utils{
    class Integer;
    namespace detail{
        const auto invalid_char = std::invalid_argument("The (C-)string( view) to be converted contains invalid characters");
        const std::string chrs = "0123456789abcdefghijklmnopqrstuvwxyz+-.\'";
        template <class T, class U>
        concept is_same_integral = std::integral<T> && std::integral<U> && std::is_same_v<T, U> && std::is_same_v<U, T>;
    }
    ...
    constexpr utils::Integer operator<<      (const utils::Integer&, std::size_t);
    ...
    inline constexpr utils::Integer pow(const utils::Integer& lhs, std::size_t rhs);
    constexpr utils::Integer pow(const utils::Integer& lhs, std::size_t rhs, const utils::Integer& mod);
}
...
template <>
struct std::hash<utils::Integer>{std::size_t operator()(const utils::Integer& value) const noexcept;};
...
class utils::Integer final{
    public:
        ...
        static constexpr std::uint8_t use_prefix = 0, binary = 2, octal = 8, decimal = 10, hex = 16, hexadecimal = 16;
        
        template <class CharT, class Traits = std::char_traits<CharT>>
        constexpr explicit Integer(const CharT* cstr, std::uint8_t base = use_prefix): Integer(std::basic_string_view<CharT, Traits>(cstr), base){}
        constexpr explicit Integer(std::nullptr_t, std::uint8_t base = use_prefix) = delete;
        template <class CharT, class Traits, class Allocator>
        constexpr explicit Integer(const std::basic_string<CharT, Traits, Allocator>& str, std::uint8_t base = use_prefix): Integer(
            std::basic_string_view<CharT, Traits>(str.data(), str.size()),
            base
        ){}
        template <class CharT, class Traits>
        constexpr explicit Integer(std::basic_string_view<CharT, Traits>, std::uint8_t = use_prefix);
        ...
        constexpr Integer& operator<<=(std::size_t);
        ...
        constexpr void swap(const Integer&);
...
constexpr utils::Integer::Integer(std::basic_string_view<CharT, Traits> sv, std::uint8_t base){
    ...
    for (const CharT& i: sv){
        char to_add = char(i);
        if (!Traits::eq(i, CharT(to_add))) throw utils::detail::invalid_char;
        if (!utils::detail::chrs.contains(to_add)) throw utils::detail::invalid_char;
        if (to_add == '\'') continue;
        if (to_add == '.') break;
        char_string += to_add;
    }
...
constexpr void utils::Integer::from_str(std::string_view sv, std::uint8_t base){
    if (sv.starts_with("++") || sv.starts_with("+-") || sv.starts_with("-+") || sv.starts_with("--")) throw utils::detail::invalid_char;
    if (sv.starts_with('-')){
        is_negative = true;
        sv.remove_prefix(1);
    }
    ...
    if (base != 10){
        for (char i: sv){
            ...
            if (digit == std::string_view::npos) throw utils::detail::invalid_char;
            *this *= base;
    ...
    for (std::size_t i = 0; i < (sv.size() / 9); i++){
        ...
        if (res.ptr != (sv.end() - 9 * i)) throw utils::detail::invalid_char;
        if (res.ec != std::errc()) throw utils::detail::invalid_char;
    }
    if (sv.size() % 9){
        digits.push_back(0);
        std::from_chars_result res = std::from_chars(sv.begin(), sv.begin() + sv.size() % 9, digits.back());
        if (res.ptr != (sv.begin() + sv.size() % 9)) throw utils::detail::invalid_char;
        if (res.ec != std::errc()) throw utils::detail::invalid_char;
    }
    correct();
}
...
constexpr std::vector<std::uint32_t> utils::Integer::subtract(const std::vector<std::uint32_t>& lhs, const std::vector<std::uint32_t>& rhs){
    ...
    for (std::size_t i = 0; i < intermediate.size(); i++) result[i] = intermediate[i];
    return result;
}
...
constexpr utils::Integer& utils::Integer::operator/=(const utils::Integer& rhs){
    if (!rhs) throw std::invalid_argument("Division by 0 is illegal"/*, the math police are after you*/);
    if (rhs == 1) return *this;
    if (rhs == -1) return *this = -*this;
    if (!*this) return *this;
...
constexpr std::basic_string<CharT, Traits, Allocator> utils::Integer::to_string_custom_alloc(const Allocator& alloc, std::uint8_t base) const{
    ...
    if (*this < 0) result += CharT('-');
    // TODO: Uncomment after division is implemented
    // if (base != 10){
...
constexpr utils::Integer        utils::operator<<(const utils::Integer& lhs, std::size_t rhs)         {
    utils::Integer result = lhs;
    return result <<= rhs;
}
