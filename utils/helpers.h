...
namespace utils{
    namespace detail{
        template <class T>
        constexpr T pow(const T&, std::integral auto);
        template <class T>
        constexpr T pow(const T&, std::unsigned_integral auto);
    }
...
template <class T>
constexpr T utils::detail::pow(const T& lhs, std::integral auto rhs){
    if (rhs >= 0) return utils::detail::pow(lhs, std::uintmax_t(rhs));
    return 1 / utils::detail::pow(lhs, std::uintmax_t(-rhs));
}
template <class T>
constexpr T utils::detail::pow(const T& lhs, std::unsigned_integral auto rhs){
